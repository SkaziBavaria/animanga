...
// Supported hosts. Lower rank wins when stream quality is equal.
const PROVIDER_RANK = {
  default: 0,
  wixmp: 0,
  'yt-mp4': 1,
...
async function fetchEpisodeSources(showId, episode, mode = 'sub') {
  ...
  for (let attempt = 0; attempt < RESOLVER_ATTEMPTS; attempt += 1) {
    ...
    const key = config.candidateKey || candidateKey(config.maskHex, config.buildId, CRYPTO_LANE_ANIME);
    ...
    const response = await fetchWithTimeout(ALLANIME_API, {
      ...
      body: JSON.stringify({
        ...
        variables,
        extensions: {
          aaReq: buildAaRequest(EPISODE_SOURCES_QUERY, config),
          k: config.lane || CRYPTO_LANE_ANIME,
        },
      }),
    });
...
function sourceProviderId(source) {
  ...
  if (/^Default$/i.test(name)) return 'Default';
  if (/^Yt-mp4$/i.test(name) || /tools\.fast4speed\.rsvp/i.test(url)) return 'Yt-mp4';
  if (/^S-mp4$/i.test(name) || /sharepoint/i.test(name) || /sharepoint/i.test(url)) return 'S-mp4';
  if (/^Mp4(?:Upload)?$/i.test(name) || /mp4upload\.com/i.test(url)) return 'Mp4Upload';
  // Encoded clock paths use AllAnime's Default host.
  if (url.startsWith('--')) return 'Default';
  return null;
}

function isAllowedSource(source) {
  return sourceProviderId(source) != null;
}

function isOkRuSource(source) {
  const name = String(source?.sourceName || '').trim();
  const url = String(source?.sourceUrl || '');
  return /^Ok(?:\.?ru)?$/i.test(name) || /ok\.ru\/videoembed\//i.test(url);
}

function byAllAnimePriority(a, b) {
  return Number(b.priority || 0) - Number(a.priority || 0);
}

async function collectResolvedLinks(sources, resolver) {
  const settled = await Promise.allSettled(sources.map(resolver));
  return {
    links: settled.filter((item) => item.status === 'fulfilled').flatMap((item) => item.value),
    failures: settled
...
function providerRank(provider) {
  const key = String(provider || '').toLowerCase().replace(/\s+/g, '');
  if (Object.prototype.hasOwnProperty.call(PROVIDER_RANK, key)) return PROVIDER_RANK[key];
  if (key.includes('default') || key.includes('wix')) return 0;
  if (key.includes('yt')) return 1;
  if (key.includes('sharepoint') || key === 's-mp4') return 2;
  if (key.includes('mp4upload') || key === 'mp4') return 3;
  return 99;
}

function comparePlaybackLinks(a, b) {
...
async function resolveSource(source) {
  ...
  if (sourceUrl.startsWith('--')) {
    const links = await resolveClockSource(sourceUrl);
    return links.map((link) => ({ ...link, provider: 'Default' }));
  }
  if (providerId === 'Mp4Upload' || /mp4upload\.com\/embed-/i.test(sourceUrl)) {
    return resolveMp4Upload(sourceUrl);
  }
  if (/\.(?:mp4|m3u8)(?:$|\?)/i.test(sourceUrl) || /tools\.fast4speed\.rsvp/i.test(sourceUrl)) {
    return [{
      url: sourceUrl,
      quality: qualityNumber(source.quality),
      referrer: providerId === 'S-mp4' ? '' : ALLANIME_REFERER,
      provider: providerId,
    }];
  }
  return [];
...
function selectQuality(links, preferred = 'best') {
  ...
  const sorted = [...usable].sort(comparePlaybackLinks);
...
async function pickPlaybackFromSources(sources, quality = 'best') {
  const preferred = [...sources.filter(isAllowedSource)].sort(byAllAnimePriority);
  const primary = await collectResolvedLinks(preferred, resolveSource);
  let selected = selectQuality(primary.links, quality);
  let failures = primary.failures;

  if (!selected) {
    const fallbackSources = [...sources.filter(isOkRuSource)].sort(byAllAnimePriority);
    const fallback = await collectResolvedLinks(fallbackSources, (source) => {
      const sourceUrl = String(source?.sourceUrl || '');
      return sourceUrl ? resolveOkRu(sourceUrl) : [];
    });
    selected = selectQuality(fallback.links, quality);
...
async function resolveEpisodePlayback({ showId, episode, mode = 'sub', quality = 'best' }) {
  ...
  return pickPlaybackFromSources(sources, quality);
}
