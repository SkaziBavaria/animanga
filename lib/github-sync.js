...
async function jsonRequest(url, options = {}) {
  const response = await rawFetcher(url, options);
...
async function ensureRepo() {
  ...
  try {
    ...
    await githubApi('/user/repos', {
      ...
      body: JSON.stringify({ name: REPO_NAME, private: true, auto_init: true, description: 'Private ani-web synchronization data.' }),
    });
  }
  return owner;
}
...
async function uploadBundle(owner, file, bundle) {
  ...
  await githubApi(`/repos/${encodeURIComponent(owner)}/${REPO_NAME}/contents/${path}`, {
    ...
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({
      message: `Sync ani-web device ${deviceId()}`,
