...
const { CURL_IMPERSONATE } = require('./config');
const {
...
function looksLikeCloudflareChallenge(body) {
  ...
  // Real catalog pages can mention challenge-platform assets; require the interstitial signals.
...
function setTextFetcherForTests(fetcher) {
...
async function fetchWebText(url, {
  ...
  if (!/^https?:\/\//i.test(url)) throw new Error('Refusing relative fetch without an absolute URL');
  const absolute = url;
