...
function mergeShow(state, partial, options = {}) {
  ...
  const { index: _browseIndex, ...rawPartial } = partial;
...
function seedStateFromHistory(state, rows = readHistory()) {
  for (const row of rows) {
    // The database is authoritative after the initial import. ani-cli can update
    // its history merely while resolving/opening a stream, so merging that row
    // again would resurrect an episode the user explicitly marked unwatched.
    if (state.shows[row.id]) continue;
    const watchedEpisodes = row.lastWatched ? [row.lastWatched] : [];
    mergeShow(state, {
      id: row.id,
      title: row.title,
      sourceName: row.name,
      ...
      latestEpisode: row.episodeCount,
      tracked: true,
      watchedEpisodes,
    });
  }
