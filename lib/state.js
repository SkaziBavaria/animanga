...
function syncShowValue(show) {
  const fields = [
    ...
    'dismissedNextSeasonId', 'sequelAlertsMuted', 'provider', 'providerId', 'hianimeId', 'legacyAnidbId',
...
function syncMatchedShowValue(show) {
  ...
  const fields = ['id', 'name', 'title', 'thumbnail', 'episodeCount', 'mode', 'provider', 'providerId', 'hianimeId'];
  return Object.fromEntries(fields.filter((key) => show[key] !== undefined).map((key) => [key, show[key]]));
}
