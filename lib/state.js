...
const ENTITY_GROUPS = ['shows', 'downloads', 'positions', 'releaseWatches'];
let database = null;
...
function syncShowValue(show) {
  const fields = [
    ...
    'thumbnail', 'mode', 'episodeCount', 'latestEpisode',
  ];
  ...
  try {
    for (const bundle of bundles || []) {
      ...
      for (const record of bundle.records) {
        if (!['show', 'tracking', 'custom_name', 'watch', 'position', 'setting', 'settings'].includes(record.kind) || !record.key || !record.updatedAt || !record.deviceId) continue;
