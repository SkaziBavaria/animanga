...
function isAuthExempt(pathname, method, searchParams = null) {
  const verb = String(method || 'GET').toUpperCase();
  if (verb === 'GET' && pathname === GOOGLE_CALLBACK_PATH) return true;
  // <video>/<img> often omit Authorization on same-origin media requests (especially
  // mobile browsers). Signed proxy URLs already carry an HMAC; proxyStream validates it.
  if (verb === 'GET' && pathname === '/api/proxy') {
    const params = searchParams && typeof searchParams.get === 'function'
      ? searchParams
      : null;
    return Boolean(params?.get('sig') && params?.get('exp') && params?.get('url'));
  }
  return false;
}
...
function requireAuthentication(req, res, pathname = '', searchParams = null) {
  if (isAuthExempt(pathname, req.method, searchParams)) return true;
