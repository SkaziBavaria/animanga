...
const { ALLANIME_REFERER } = require('./config');
...
const MANGA_REFERRER = 'https://allmanga.to/';
...
function buildProxyPath(targetUrl, referrer = ALLANIME_REFERER, { ttlSeconds = PROXY_TTL_MEDIA_SECONDS } = {}) {
...
function proxyRemotePages(result, referrer = MANGA_REFERRER) {
  return {
    ...
    pages: (result.pages || []).map((page) => {
      ...
      return {
        ...page,
        url: buildProxyPath(pageUrl, referrer, { ttlSeconds: PROXY_TTL_MEDIA_SECONDS }),
      };
    }),
  };
}
