...
async function searchManga(search = '', {
  ...
  const data = await graphql(query, {
    search: input,
    limit: rangeDays ? Math.max(limit, 100) : limit,
    page,
    translationType: language,
    countryOrigin: 'ALL',
  });
  let results = (data?.mangas?.edges || []).filter((item) => item?._id).map((item) => mangaSummary(item, language));
  if (rangeDays) {
    const cutoff = Date.now() - (Number(rangeDays) * 86_400_000);
    results = results.filter((item) => mangaDate(item.lastChapterDate)?.getTime() >= cutoff).slice(0, limit);
  }
  return {
...
function mangaDate(value) {
  ...
  if (!year) return null;
  return new Date(Date.UTC(year, Math.max(0, Number(value.month || 1) - 1), Number(value.date || 1), Number(value.hour || 0), Number(value.minute || 0)));
}
...
async function getMangaDetails(id, language = 'sub') {
  const query = `query($_id: String!) {
    manga(_id: $_id) {
      ${SUMMARY_FIELDS}
      description thumbnails authors genres status altNames averageScore rating banner airedEnd magazine
...
async function getRelatedMangas(rawRelations, language) {
  ...
  const fields = relations.map((item, index) => `r${index}: manga(_id: ${JSON.stringify(item.mangaId)}) { ${SUMMARY_FIELDS} status averageScore }`).join('\n');
  try {
...
async function getChapterPages(id, chapter, language = 'sub') {
  ...
  return { pages, notes: source.notes || '', sourceName: source.sourceName || '' };
}

module.exports = { searchManga, getMangaDetails, getChapterPages, mangaSummary, coverUrl, setRawFetcher };
