...
let cryptoMaterial = null;
let cachedClientCrypto = null;
let cryptoRefresh = null;
...
async function resolveClientCrypto(appEntryUrl, { force = false } = {}) {
  if (!force && cachedClientCrypto?.maskHex && cachedClientCrypto?.buildId) return cachedClientCrypto;
  if (!appEntryUrl) {
    if (cachedClientCrypto) return cachedClientCrypto;
  ...
  const skipMaskHex = force ? cachedClientCrypto?.maskHex || '' : '';

  for (const name of chunkNames.slice(0, MAX_MASK_CHUNKS)) {
    const body = await fetchText(chunkBase + name).catch(() => '');
    const extracted = extractClientCrypto(body, { skipMaskHex });
    if (!extracted) continue;
    cachedClientCrypto = extracted;
    return extracted;
  }
...
async function mangaCryptoConfig({ forceRefresh = false, healMask = false } = {}) {
  if (!forceRefresh && cryptoMaterial && Date.now() < cryptoMaterial.expiresAt) return cryptoMaterial;
  if (cryptoRefresh) return cryptoRefresh;

  cryptoRefresh = (async () => {
    const html = await fetchText(`${MKISSA_ORIGIN}/`);
    const bootstrapJson = html.match(AA_CRYPTO_REGEX)?.[1];
    if (!bootstrapJson) throw new Error('Unable to obtain AllManga crypto bootstrap');
    const bootstrap = JSON.parse(bootstrapJson);
    if (!bootstrap?.partB || !Number.isFinite(Number(bootstrap.epoch))) {
      throw new Error('AllManga crypto bootstrap is incomplete');
    }

    const appEntryUrl = html.match(APP_ENTRY_REGEX)?.[1] || null;
    const client = await resolveClientCrypto(appEntryUrl, { force: healMask || !cachedClientCrypto });
    const key = deriveMangaKey(client.maskHex, bootstrap.partB);
    cryptoMaterial = {
  ...
  try {
    return await cryptoRefresh;
  } finally {
    cryptoRefresh = null;
  }
}

function invalidateMangaCrypto({ healMask = false } = {}) {
  cryptoMaterial = null;
  if (healMask) cachedClientCrypto = null;
}
...
function aaRequest(query, config) {
  ...
  const payload = JSON.stringify({ v: 1, ts, epoch: config.epoch, buildId: String(config.buildId), qh });
...
async function encryptedGraphql(query, variables) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const healMask = attempt >= 2;
    if (attempt > 0) invalidateMangaCrypto({ healMask });
    const config = await mangaCryptoConfig({ forceRefresh: attempt > 0, healMask });
    const response = await fetch(ALLANIME_API, {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
    ...
    if (isAaCryptoError(result)) {
      lastError = new Error(result.errors.map((error) => error.message).join('; '));
      continue;
    }
    if (result.errors?.length) throw new Error(result.errors.map((error) => error.message).join('; '));
