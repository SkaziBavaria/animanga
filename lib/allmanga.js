...
const { isRateLimitError, withUpstreamRetry } = require('./upstream-retry');
const {
  MKISSA_ORIGIN,
  CHAPTER_PAGES_QUERY,
  CRYPTO_LANE_MANGA,
...
const POPULAR_FIELDS = `
  _id name englishName nativeName thumbnail lastChapterInfo lastChapterDate
  chapterCount type season score airedStart availableChapters lastUpdateEnd slugTime status averageScore airedEnd
`;
...
async function fetchRaw(query, variables) {
  return withUpstreamRetry(async () => {
    ...
    if (response.status === 429) throw new Error('AllManga API rate limit (429)');
...
async function graphql(query, variables) {
  const result = await withUpstreamRetry(async () => {
    const payload = await rawFetcher(query, variables);
    if (payload.errors?.length) {
      const message = payload.errors.map((error) => error.message).join('; ');
      if (isRateLimitError(message)) throw new Error(message);
...
async function encryptedGraphql(query, variables) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    const key = config.candidateKey || candidateKey(config.maskHex, config.buildId, CRYPTO_LANE_MANGA);
    ...
    const result = await withUpstreamRetry(async () => {
      const response = await fetchWithTimeout(fetch, ALLANIME_API, {
        ...
        body: JSON.stringify({
          query,
          variables,
          extensions: { aaReq: aaRequest(query, config), k: config.lane || CRYPTO_LANE_MANGA },
        }),
      });
      if (response.status === 429) throw new Error('AllManga chapter API rate limit (429)');
      if (!response.ok) throw new Error(`AllManga chapter API returned ${response.status}`);
      const payload = await response.json();
      if (payload.errors?.length && !isAaCryptoError(payload)) {
        const message = payload.errors.map((error) => error.message).join('; ');
        if (isRateLimitError(message)) throw new Error(message);
        throw new Error(message);
      }
...
async function popularManga(range = 0, { limit = 30, language = 'sub' } = {}) {
  ...
  const results = recommendations
    .filter((item) => item?.isManga !== false && item?.anyCard?._id)
    .map((item) => mangaSummary(item.anyCard, language));
  return {
    total: Number(data?.queryPopular?.total || recommendations.length),
    results: await enrichMangaOrigins(results),
  };
}

/** Popular AnyCard omits countryOfOrigin; fill it via a batched manga(_id) lookup. */
async function enrichMangaOrigins(results) {
  const missing = results.filter((item) => item?.id && !item.countryOfOrigin);
  if (!missing.length) return results;
  const fields = missing
    .map((item, index) => `o${index}: manga(_id: ${JSON.stringify(item.id)}) { _id countryOfOrigin }`)
    .join('\n');
  try {
    const payload = await rawFetcher(`query { ${fields} }`, {});
    if (payload.errors?.length) return results;
    const data = payload.data || {};
    const byId = new Map();
    for (let index = 0; index < missing.length; index += 1) {
      const manga = data[`o${index}`];
      if (manga?._id) byId.set(String(manga._id), manga.countryOfOrigin || '');
    }
    if (!byId.size) return results;
    return results.map((item) => (
      byId.has(String(item.id))
        ? { ...item, countryOfOrigin: byId.get(String(item.id)) || '' }
        : item
    ));
  } catch {
    return results;
...
async function getMangaDetails(id, language = 'sub', { includeRelations = true } = {}) {
  ...
  const relations = includeRelations
    ? await getRelatedMangas(manga.relatedMangas || [], language)
    : [];
  return {
