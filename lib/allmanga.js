...
function aaRequest(query, config) {
  ...
  const payload = JSON.stringify({ v: 1, ts, epoch: Number(config.epoch), buildId: String(config.buildId), qh });
...
async function encryptedGraphql(query, variables) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (result.errors?.length) throw new Error(result.errors.map((error) => error.message).join('; '));
    const data = result.data?.tobeparsed ? decryptResponse(result.data.tobeparsed, config) : result.data;
