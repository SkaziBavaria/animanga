...
async function refreshKind(kind, now, dependencies = {}) {
  ...
  for (const item of due) {
    try {
      ...
      recordProviderFailure(kind === 'anime' ? 'hianime' : 'comick', error, { retryAt: blockedUntil[kind], now });
...
function resetLibraryRefreshForTests() {
  blockedUntil.anime = 0;
  blockedUntil.manga = 0;
  running = null;
}
