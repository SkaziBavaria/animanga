...
function titleScore(query, candidate) {
  ...
  if (overlap === qTokens.size && overlap === nTokens.size) return 90;
  if (overlap === qTokens.size) return 70;
...
async function migrateShowEntry(state, show) {
  ...
  try {
    const results = await searchAnime(query, show.mode || state.settings?.mode || 'sub', { filterRelevance: false });
