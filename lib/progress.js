...
function mangaPositionKey(mangaId, language, chapter) {
  return `${mangaId}:${language === 'raw' ? 'raw' : 'sub'}:${String(chapter || '').trim()}`;
}

function setMangaPosition(state, { id, mangaId, language, chapter, page, pageCount, clear = false }) {
  ...
  const key = mangaPositionKey(resolvedId, language, normalizedChapter);
