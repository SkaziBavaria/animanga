...
async function runMangaDownloadTask(item) {
  const { job, chapter, language } = item;
  ...
  try {
    ...
    let result = readManifest(job.mangaId, language, chapter);
    if (!result) result = await getChapterPages(job.mangaId, chapter, language);
    await downloadChapter(job.mangaId, language, chapter, result);
    job.completed += 1;
  } catch (err) {
    job.failed += 1;
...
function startMangaDownloadBatch({ mangaId, mangaName, language = 'sub', chapters }) {
  ...
  const pending = requested.filter((chapter) => !readManifest(mangaId, language, chapter)?.downloaded);
  ...
  pending.forEach((chapter) => registry.mangaDownloadQueue.push({ job, chapter, language }));
