...
const { DEFAULT_MEDIA_REFERER, USER_AGENT } = require('./config');
...
function isVttTarget(contentType, targetUrl) {
  const type = String(contentType || '').toLowerCase().split(';')[0].trim();
  if (type === 'text/vtt') return true;
  try {
    return new URL(String(targetUrl || '')).pathname.toLowerCase().endsWith('.vtt');
  } catch {
    return /\.vtt(?:$|[?#])/i.test(String(targetUrl || ''));
  }
}
...
async function proxyStream(req, res, url) {
  ...
  const referrer = url.searchParams.get('referrer') || DEFAULT_MEDIA_REFERER;
  ...
  try {
    ...
    const responseHeaders = {
      'content-type': isVttTarget(upstreamContentType, rawTarget)
