...
function parseDecoderIndexExpr(expr) {
  const match = String(expr || '').trim().match(/^([a-zA-Z_$][\w$]*)\s*-\s*(-?\d+)$/);
  if (!match) throw new Error(`mkissa decoder index expression is invalid: ${expr}`);
  return { arg: match[1], offset: Number(match[2]) };
}

function detectMaskFragmentScheme(source) {
  if (!source || !source.includes('aaReq')) return null;
  if (source.includes('nd=[') && source.includes('function ml()')) {
    return {
      tableFn: 'ml',
      decoderFn: 'Na',
      leftFn: 'Hr',
  ...
  if (source.includes('Ju=[') && source.includes('function ll()')) {
    return {
      tableFn: 'll',
      decoderFn: 'Ca',
      leftFn: 'Rr',
...
function extractDecoderOffset(source, scheme) {
  const { tableFn, decoderFn } = scheme;
  const match = source.match(new RegExp(
    `function ${decoderFn}\\([^)]*\\)\\{return e=e-\\(([^)]+)\\),${tableFn}\\(\\)\\[e\\]\\}`
  )) || source.match(new RegExp(
    `function ${decoderFn}\\([^)]*\\)\\{return e=e-([^,]+),${tableFn}\\(\\)\\[e\\]\\}`
  ));
  if (!match) throw new Error(`mkissa ${decoderFn} offset is missing`);
  return Function(`"use strict"; return (${match[1]});`)();
}

function rotateStringTable(source, table, scheme) {
  const { tableFn, decoderFn } = scheme;
  const callMarker = `})(${tableFn},`;
  const callAt = source.indexOf(callMarker);
  if (callAt < 0) throw new Error(`mkissa ${tableFn} shuffle is missing`);

  let fnAt = source.lastIndexOf('(function(e,t){', callAt);
  if (fnAt < 0) throw new Error(`mkissa ${tableFn} shuffle is missing`);
  // Prefer the shuffle IIFE closest to the call site.
  const nested = source.lastIndexOf('(function(e,t){', callAt - 1);
  if (nested > fnAt && nested < callAt) fnAt = nested;

  const head = source.slice(fnAt, callAt);
  const helperMatches = [...head.matchAll(new RegExp(
    `function ([a-zA-Z_$][\\w$]*)\\([^)]*\\)\\{return ${decoderFn}\\(([^)]+)\\)\\}`,
    'g'
  ))];
  if (helperMatches.length < 2) throw new Error(`mkissa ${tableFn} shuffle helpers are missing`);

  // Legacy shape used r/a; current shape uses n/a. Keep first two unique helpers.
  const helpers = [];
  const seen = new Set();
  for (const match of helperMatches) {
    if (seen.has(match[1])) continue;
    seen.add(match[1]);
    helpers.push({ name: match[1], ...parseDecoderIndexExpr(match[2]) });
    if (helpers.length === 2) break;
  }
  if (helpers.length < 2) throw new Error(`mkissa ${tableFn} shuffle helpers are incomplete`);

  const ifAt = head.indexOf('try{if(');
  const eqAt = head.lastIndexOf('===t)break;');
  if (ifAt < 0 || eqAt < 0) throw new Error(`mkissa ${tableFn} shuffle expression is missing`);
  const expr = head.slice(ifAt + 'try{if('.length, eqAt);
  const targetStart = callAt + callMarker.length;
  const targetEnd = source.indexOf(')', targetStart);
  if (targetEnd < 0) throw new Error(`mkissa ${tableFn} shuffle target is missing`);
  const target = Function(`"use strict"; return (${source.slice(targetStart, targetEnd)});`)();
  const decoderOffset = extractDecoderOffset(source, scheme);

  function decode(index) {
    return table[index - decoderOffset];
  }
  ...
  for (;;) {
    try {
      ...
      for (const helper of helpers) {
        bindings[helper.name] = (_first, second) => {
          const value = helper.arg === 'e' || helper.arg === 'i' ? _first : second;
          return decode(value - helper.offset);
        };
      }
      const names = helpers.map((helper) => helper.name);
      const value = Function(...names, `"use strict"; return (${expr});`)(
...
function buildFragmentLookup(source, table, decoderOffset, scheme) {
  const { decoderFn, leftFn, rightFn } = scheme;
  const leftMatch = source.match(new RegExp(
    `function ${leftFn}\\([^)]*\\)\\{return ${decoderFn}\\(([^)]+)\\)\\}`
  ));
  const rightMatch = source.match(new RegExp(
    `function ${rightFn}\\([^)]*\\)\\{return ${decoderFn}\\(([^)]+)\\)\\}`
  ));
  if (!leftMatch || !rightMatch) return null;

  const leftInfo = parseDecoderIndexExpr(leftMatch[1]);
  const rightInfo = parseDecoderIndexExpr(rightMatch[1]);

  function decode(index) {
    return table[index - decoderOffset];
  }
  function resolve(info, first, second) {
    const value = info.arg === 'e' || info.arg === 'i' ? first : second;
    return decode(value - info.offset);
  }

  return {
    left(first, second) {
      // Older Rr(value) only passed one arg; newer Hr(e,t) uses the second.
      if (arguments.length < 2) return resolve(leftInfo, first, first);
      return resolve(leftInfo, first, second);
    },
    right(first, second) {
      return resolve(rightInfo, first, second);
    },
  };
}

function extractJuCandidate(source) {
  const scheme = detectMaskFragmentScheme(source);
  if (!scheme) return null;

  const arrayLiteral = extractArrayLiteral(source, `function ${scheme.tableFn}(){const e=[`);
  if (!arrayLiteral) return null;
  const table = Function(`"use strict"; return (${arrayLiteral});`)().slice();
  const decoderOffset = rotateStringTable(source, table, scheme);
  const lookup = buildFragmentLookup(source, table, decoderOffset, scheme);
  if (!lookup) return null;

  const fragmentMatch = source.match(new RegExp(`${scheme.fragmentsVar}=\\[([^\\]]+)\\]`));
  if (!fragmentMatch) return null;
  const chunks = Function(
    scheme.leftFn,
    scheme.rightFn,
    `"use strict"; return [${fragmentMatch[1]}];`
