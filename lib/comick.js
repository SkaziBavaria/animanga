...
async function searchManga(search = '', {
  ...
  const windowDays = Math.trunc(Number(time));
  if (Number.isFinite(windowDays) && windowDays > 0) params.set('time', String(Math.min(365, windowDays)));
...
const MANGA_POPULAR_WINDOWS = {
  0: { sortBy: 'follow' },
  1: { sortBy: 'view', time: 1 },
  7: { sortBy: 'view', time: 7 },
  30: { sortBy: 'view', time: 30 },
};

async function popularManga(range = 0, { limit = 30 } = {}) {
  const window = MANGA_POPULAR_WINDOWS[Number(range) || 0];
  if (!window) throw Object.assign(new Error('Unknown catalog browse'), { status: 422 });
  return searchManga('', { limit, page: 1, ...window });
}
