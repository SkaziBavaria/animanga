...
async function getShowSummaryById(id, mode = 'sub') {
  ...
  const normalizedMode = normalizeMode(mode);
  const episodeCount = show.availableEpisodes?.[normalizedMode] || show.episodeCount || 0;
...
async function getShowDetails(id, mode = 'sub', options = {}) {
  ...
  const episodeCount = show.availableEpisodes?.[normalizedMode] || episodes.length;
  const name = preferredName(show);
  const lastInfo = show.lastEpisodeInfo?.[normalizedMode] || show.lastEpisodeInfo?.sub || show.lastEpisodeInfo?.raw || {};
