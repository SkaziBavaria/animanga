...
async function softFetchSequelSummaries(ids, mode = 'sub') {
  const byId = new Map();
...
async function resolveSequelSummaries(ids, mode = 'sub') {
  ...
  if (stillMissing.length) {
    const recovered = await mapConcurrent(stillMissing, 2, async (id) => {
      try {
        return await getShowSummaryById(id, mode);
      } catch {
        return null;
      }
    });
    stillMissing.forEach((id, index) => {
      const summary = recovered[index];
      if (!summary?.id) return;
      writeSequelCache(id, mode, summary);
      byId.set(id, summary);
    });
  }
  return byId;
...
async function enrichNextSeasons(results, mode = 'sub') {
  ...
  // Inline soft-fetch for small result sets (typical search). Popular (~35 sequels)
  // relies on cache + /api/sequels hydration so the list can render immediately.
