...
async function listSyncFiles() {
  ...
  url.searchParams.set('q', "name contains 'animanga-sync-' and trashed = false");
...
async function performSync() {
  ...
  const ownName = `animanga-sync-${deviceId()}.json`;
  const ownFile = files.find((file) => file.name === ownName);
