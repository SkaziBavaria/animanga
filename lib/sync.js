...
async function finishAuthorization({ code, state, callbackUrl }) {
  const value = config();
  ...
  const token = await tokenRequest({
    ...
    redirect_uri: callbackUrl || value.callbackUrl,
  ...
  return publicConfig(callbackUrl);
}
...
async function listSyncFiles() {
  ...
  url.searchParams.set('q', "name contains 'ani-web-sync-' and trashed = false");
...
async function uploadBundle(file, bundle) {
  ...
  if (!id) {
    const created = await driveFetch(`${DRIVE_API}/files`, {
      ...
      body: JSON.stringify({
        name: `ani-web-sync-${deviceId()}.json`,
...
async function performSync() {
  ...
  const ownName = `ani-web-sync-${deviceId()}.json`;
  await uploadBundle(files.find((file) => file.name === ownName), syncBundle());
  const lastSyncAt = new Date().toISOString();
  saveConfig({ lastSyncAt, lastSyncError: '' });
  return { ok: true, lastSyncAt, applied: merged.applied, files: files.length + (files.some((file) => file.name === ownName) ? 0 : 1) };
}
