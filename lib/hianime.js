...
function subtitleEntriesFromConfig(config) {
  const lists = [config?.subtitles, config?.tracks];
  return lists.flatMap((list) => (Array.isArray(list) ? list : []));
}

function listSubtitleTracks(config, baseUrl = HIANIME_ORIGIN, referrer = '') {
  const tracks = [];
  const seen = new Set();
  for (const item of subtitleEntriesFromConfig(config)) {
    if (!item || /thumb/i.test(String(item.kind || item.type || item.label || ''))) continue;
    const src = absoluteUrl(item.src || item.file || item.url, baseUrl);
    if (!src || seen.has(src)) continue;
    seen.add(src);
    const lang = String(item.lang || item.language || '').trim().slice(0, 16);
    const label = String(item.label || lang || 'Captions').trim().slice(0, 64) || 'Captions';
    tracks.push({
      src,
      lang,
      label,
...
function mergeSubtitleTracks(...groups) {
  ...
  for (const track of groups.flat()) {
    if (!track?.src || bySrc.has(track.src)) continue;
    bySrc.add(track.src);
    const key = String(track.label || track.lang || '').trim().toLowerCase() || track.src;
    const existing = byLabel.get(key);
    if (!existing || (!existing.default && track.default)) byLabel.set(key, track);
  }
  return [...byLabel.values()];
}

function pickDefaultSubtitleTrack(tracks) {
  const usable = (Array.isArray(tracks) ? tracks : []).filter((item) => item?.src);
  return usable.find((item) => item.default) || usable[0] || null;
}

function pickSubtitleTrack(config, baseUrl = HIANIME_ORIGIN, referrer = '') {
  return pickDefaultSubtitleTrack(listSubtitleTracks(config, baseUrl, referrer));
}

function resolveDefaultSubtitleTrack(primaryTracks, mergedTracks) {
  return pickDefaultSubtitleTrack(primaryTracks) || pickDefaultSubtitleTrack(mergedTracks);
}

function withResolvedSubtitle(link, track, serverName, tracks = []) {
  const selected = track || pickDefaultSubtitleTrack(tracks);
  const subtitleTracks = tracks.map((item) => ({
    ...item,
    default: Boolean(selected && item.src === selected.src),
  }));
  return {
    ...link,
    server: serverName,
    ...
    ...(selected ? {
      subtitle: selected.src,
      subtitleLang: selected.lang,
      subtitleLabel: selected.label,
      ...(selected.referrer ? { subtitleReferrer: selected.referrer } : {}),
    } : {}),
  };
}
...
async function resolveEpisodeLinks(showId, episode, mode = 'sub') {
  ...
  let extraCaptionPromise = null;
  const loadExtraCaptionTracks = () => {
    if (!extraCaptionPromise) {
      extraCaptionPromise = (async () => {
        for (const server of parseMegaplayCaptionServers(servers, language)) {
          try {
            ...
            const tracks = listSubtitleTracks(captionConfig, captionConfig?.baseUrl, captionConfig?.referrer);
            if (tracks.length) return tracks;
          } catch {
            // Extra caption tracks are optional; keep the playable stream.
          }
        }
        return [];
      })();
    }
    return extraCaptionPromise;
  };
  loadExtraCaptionTracks();
  let lastError = null;
  for (const server of serversForLanguage.slice(0, 4)) {
    try {
      ...
      const primaryTracks = listSubtitleTracks(config, embedUrl, referrer);
      const tracks = mergeSubtitleTracks(primaryTracks, await loadExtraCaptionTracks());
      const track = resolveDefaultSubtitleTrack(primaryTracks, tracks);
      if (!links.length) {
        if (!/#EXTINF:/.test(playlist)) throw new Error('HiAnime returned an empty playlist');
        return [withResolvedSubtitle({ url: masterUrl, quality: null, referrer, provider: 'hianime' }, track, server.name, tracks)];
      }
      return links.map((link) => withResolvedSubtitle(link, track, server.name, tracks));
    } catch (error) {
      lastError = error;
    }
  }
