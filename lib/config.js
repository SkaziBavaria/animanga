...
const LEGACY_LOCAL_DATA_DIR = path.join(ROOT, 'data');
const DEFAULT_DATA_DIR = fs.existsSync(LEGACY_LOCAL_DATA_DIR) ? LEGACY_LOCAL_DATA_DIR : userDataDir();
const DATA_DIR = path.resolve(process.env.ANIMANGA_DATA_DIR || process.env.ANI_WEB_DATA_DIR || DEFAULT_DATA_DIR);
...
const DOWNLOAD_DIR = path.resolve(
  process.env.ANIMANGA_DOWNLOAD_DIR
  || process.env.ANI_CLI_DOWNLOAD_DIR
...
const ANIME_RESOLVER = String(process.env.ANIMANGA_ANIME_RESOLVER || 'node').toLowerCase() === 'ani-cli' ? 'ani-cli' : 'node';
const ANI_CLI_FALLBACK = process.env.ANIMANGA_ANI_CLI_FALLBACK === '1';
