...
const { migrateLibraryToComicK, isComicKManga } = require('../comick-migrate');

let libraryMigrationPromise = null;

async function ensureMangaLibraryMigrated(state) {
  const needs = Object.values(state.mangas || {}).some((manga) => !isComicKManga(manga));
  if (!needs) return null;
  if (!libraryMigrationPromise) {
    libraryMigrationPromise = migrateLibraryToComicK(state)
      .then((report) => {
        if (report.migrated || report.needsRematch) saveState(state);
        return report;
      })
      .finally(() => { libraryMigrationPromise = null; });
  }
...
async function handleMangaRoutes(req, res, url) {
  if (req.method === 'GET' && url.pathname === '/api/manga/library') {
    ...
    await ensureMangaLibraryMigrated(state);
    ...
    mangas = mangas.map((manga) => ({
      ...manga,
      downloadedChapters: listDownloads(manga.id, 'sub').length,
    }));
  ...
  if (req.method === 'GET' && url.pathname === '/api/manga/recommendations') {
    ...
    const payload = presentMangaResults(await searchManga('', { genres: ranked, limit: 30 }));
    const reason = ranked.length
      ? `Because you read ${ranked.slice(0, 2).join(' + ')}`
      : 'Popular with manga readers';
    payload.results = (payload.results || []).map((item) => ({ ...item, recommendationReason: reason }));
  ...
  if (req.method === 'POST' && url.pathname === '/api/manga/track') {
    ...
    const state = readState();
    let details;
    try { details = await getMangaDetails(body.id); } catch {}
  ...
  if (req.method === 'POST' && url.pathname === '/api/manga/read-through') {
    ...
    let details;
    try {
      details = await getMangaDetails(body.id);
    } catch {
      details = {};
    }
  ...
  if (req.method === 'GET' && mangaChaptersMatch) {
    ...
    const state = readState();
    const existing = state.mangas[id] || {};
    const details = await getMangaDetails(id, { includeRelations: false });
    const manga = existing.id ? mergeManga(state, { ...existing, ...details }) : { ...existing, ...details };
    if (existing.id) saveState(state);
    const chapterDates = cachedChapterDates(id, 'sub');
  ...
  if (req.method === 'GET' && mangaDownloadsMatch) {
    ...
    return sendJson(res, 200, { downloads: listDownloads(id, 'sub') });
  }
  ...
  if (mangaDownloadMatch && ['POST', 'DELETE'].includes(req.method)) {
    ...
    if (req.method === 'DELETE') {
      return sendJson(res, 200, { deleted: deleteDownload(id, 'sub', chapter) });
    }
    let result = readManifest(id, 'sub', chapter);
    if (!result || result.downloaded) result = await getChapterPages(id, chapter);
    const manifest = await downloadChapter(id, 'sub', chapter, result);
  ...
  if (req.method === 'GET' && mangaLocalPageMatch) {
    ...
    const page = localPage(id, 'sub', chapter, number);
  ...
  if (req.method === 'GET' && mangaPagesMatch) {
    ...
    const cached = readManifest(id, 'sub', chapter);
    if (cached?.downloaded) return sendJson(res, 200, presentPages(id, 'sub', chapter, cached));
    try {
      const result = await getChapterPages(id, chapter);
      cacheChapter(id, 'sub', chapter, result);
      ...
      if (cached?.pages?.length) {
        ...
        if (isVerifiedRemoteManifest(cached, id, names)) {
          return sendJson(res, 200, proxyRemotePages(cached));
        }
      }
      throw error;
    }
  ...
  if (req.method === 'PATCH' && mangaMatch) {
    ...
    if (Object.hasOwn(body, 'dismissedSequelIds')) {
      const dismissed = stringArray(body, 'dismissedSequelIds', {
        maxLength: 200,
      });
      if (dismissed.length > 100) invalid('Too many dismissed manga sequels');
      patch.dismissedSequelIds = Array.from(new Set(dismissed));
    }
  ...
  if (req.method === 'POST' && url.pathname === '/api/manga/release-watches') {
    ...
    const watch = createMangaReleaseWatch(state, query);
    saveState(state);
