...
const { catalogFieldsFromClient } = require('../library-identity');
const {
  touchShow,
...
async function handleAnimeRoutes(req, res, url) {
  ...
  if (req.method === 'GET' && (url.pathname === '/api/catalog/search' || url.pathname === '/api/hianime/search')) {
  ...
  if (req.method === 'GET' && episodeMatch) {
    ...
    try {
      const details = await animeProvider.details(state, { id, provider: url.searchParams.get('provider') }, mode, { enrichEpisodeTitles: true });
  ...
  const catalogMatch = url.pathname.match(/^\/api\/shows\/([^/]+)\/(?:catalog|hianime)$/);
  if (req.method === 'POST' && catalogMatch) {
    const id = decodeURIComponent(catalogMatch[1]);
    const body = await readBody(req);
    const providerId = requiredString(body, body.providerId ? 'providerId' : 'hianimeId', { maxLength: 200 });
    const state = readState();
    const query = body.query ? requiredString(body, 'query', { maxLength: 200 }) : '';
    const show = await matchCatalogTitle(state, id, providerId, query);
    saveState(state);
    return sendJson(res, 200, { show: presentShow(show) });
  }
  ...
  if (req.method === 'PATCH' && showMatch) {
    ...
    if (Object.hasOwn(body, 'mode')) {
      ...
      const details = await animeProvider.details(state, id, mode, { enrichEpisodeTitles: true });
