...
const { migrateLibraryToAnidb } = require('../anidb-migrate');
const { isAnidbShowId } = require('../anidb');

let libraryMigrationPromise = null;

async function ensureLibraryMigrated(state) {
  const needs = Object.values(state.shows || {}).some((show) => !isAnidbShowId(show.id) || show.needsRematch);
  if (!needs) return null;
  if (!libraryMigrationPromise) {
    libraryMigrationPromise = migrateLibraryToAnidb(state)
      .then((report) => {
...
async function handleAnimeRoutes(req, res, url) {
  if (req.method === 'GET' && url.pathname === '/api/library') {
    ...
    await ensureLibraryMigrated(state);
  ...
  if (req.method === 'POST' && url.pathname === '/api/sequels') {
    ...
    const byId = await resolveSequelSummaries(ids.slice(0, 40), mode);
    const sequels = {};
  ...
  if (req.method === 'POST' && url.pathname === '/api/track') {
    ...
    const state = readState();
    const show = mergeShow(state, { ...body, tracked: body.tracked ?? true, archived: false });
    saveState(state);
