...
async function handlePlaybackRoutes(req, res, url) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/play') {
    ...
    // Browser play sends resolveOnly. Don't gate the stream on a slow details fetch —
    // AllAnime timeouts were leaving the player closed after "Fetching stream...".
    if (!body.resolveOnly) {
      ...
      return sendJson(res, 200, {
        ...
        playback: {
          ...
          proxyUrl: buildProxyPath(
            playback.url,
            playback.referrer || ALLANIME_REFERER,
