...
const { fetchLiveCryptoConfig, formatCryptoFailure } = require('./mkissa-crypto');
 ...
 */
async function refreshAniCliCrypto({ aniCliPath = resolveAniCliPath(), fetchConfig = fetchLiveCryptoConfig } = {}) {
  ...
  try {
    ...
    try {
      ...
      if (process.platform !== 'win32') {
        // rename should work on Linux; surface unexpected failures after restore attempt.
        throw renameError;
      }
    }
    ...
    // Ensure original remains if temp replace partially failed.
    if (!fs.existsSync(target)) {
