...
const { APP_VERSION, nextReleaseVersion, installApiMocks } = require('./fixtures');

const NEWER_VERSION = nextReleaseVersion(APP_VERSION);
...
test.describe('Shell & navigation', () => {
  ...
  test('loads the shell and renders the library', async ({ page }) => {
    ...
    await expect(page.locator('#statusText')).toContainText(`v${APP_VERSION}`);
    ...
    await installApiMocks(page, {
      status: {
        ...
        update: {
          ...
          url: `https://github.com/SkaziBavaria/animanga/releases/tag/v${NEWER_VERSION}`,
          hint: `Update available (${NEWER_VERSION}). Run: npm install -g animanga@latest`,
        },
        deps: { node: 'test', mpv: false, androidActivityManager: false, clientPlayback: true, animeResolver: 'node' },
      },
    });
    await page.goto('/');
    await expect(page.locator('#statusText')).toHaveText(`v${APP_VERSION}`);
    await expect(page.locator('#updateNotice')).toBeVisible();
    await expect(page.locator('#updateNotice')).toHaveText(`Update ${NEWER_VERSION}`);
    await expect(page.locator('#updateNotice')).toHaveAttribute(
      'href',
      `https://github.com/SkaziBavaria/animanga/releases/tag/v${NEWER_VERSION}`,
    );
  });
