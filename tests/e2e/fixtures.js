...
function nextReleaseVersion(version = APP_VERSION) {
...
/** Mirror server presentManga: remote covers go through /api/proxy. */
function presentMockManga(manga) {
  ...
  return {
    ...manga,
    thumbnail: proxy(manga.thumbnail),
    thumbnails: Array.isArray(manga.thumbnails) ? manga.thumbnails.map(proxy) : manga.thumbnails,
    relations: Array.isArray(manga.relations)
      ? manga.relations.map((item) => ({ ...item, thumbnail: proxy(item.thumbnail) }))
      : manga.relations,
    matchedManga: manga.matchedManga ? presentMockManga(manga.matchedManga) : manga.matchedManga,
  };
}
...
async function installApiMocks(page, overrides = {}) {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    // --- read/health ---
    if (p === '/api/status') {
      return route.fulfill(jsonBody(status));
    }
    ...
    if (p === '/api/manga/library' && method === 'GET') {
      return route.fulfill(jsonBody({ mangas: mangaLibrary.map(presentMockManga) }));
    }
    if (p === '/api/manga/search' && method === 'GET') {
      return route.fulfill(jsonBody({ total: mangaResults.length, results: mangaResults.map(presentMockManga) }));
    }
    if (p === '/api/manga/popular' && method === 'GET') {
      return route.fulfill(jsonBody({ total: mangaResults.length, results: mangaResults.map(presentMockManga) }));
    }
    if (p === '/api/manga/recommendations' && method === 'GET') {
      return route.fulfill(jsonBody({ total: mangaResults.length, results: mangaResults.map(presentMockManga) }));
    }
    if (p === '/api/manga/release-watches' && method === 'GET') {
      return route.fulfill(jsonBody({
        watches: mangaReleaseWatches.map((watch) => ({
          ...watch,
          matchedManga: watch.matchedManga ? presentMockManga(watch.matchedManga) : watch.matchedManga,
        })),
      }));
    }
    ...
    if (p === '/api/manga/read-through' && method === 'POST') {
      ...
      mangaLibrary = mangaLibrary.map((item) => item.id === manga.id ? manga : item);
      return route.fulfill(jsonBody({ manga: presentMockManga(manga), marked }));
    }
    ...
    if (/^\/api\/manga\/[^/]+$/.test(p) && method === 'DELETE') {
      ...
      return route.fulfill(jsonBody({ manga: presentMockManga({ ...MANGA, tracked: false }) }));
    }
